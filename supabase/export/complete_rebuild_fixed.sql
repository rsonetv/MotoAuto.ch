-- Complete schema rebuild to fix type incompatibility issues
-- First drop all tables and functions in the correct order to avoid dependency issues

-- Drop all tables that might have dependency issues
...
DROP FUNCTION IF EXISTS public.update_listing_bid(UUID, DECIMAL, UUID) CASCADE;
DROP FUNCTION IF EXISTS public.search_listings(TEXT, TEXT, DECIMAL, DECIMAL, TEXT, INTEGER, INTEGER, INTEGER, INTEGER) CASCADE;
...
-- Create listings table with EXPLICIT UUID primary key
...
CREATE TABLE public.contact_categories (
  ...
  CONSTRAINT contact_categories_slug_key UNIQUE (slug)
);
...
INSERT INTO public.categories (id, name, slug, description, icon, sort_order) VALUES
  (gen_random_uuid(), 'Samochody', 'auta', 'Samochody osobowe, terenowe, sportowe i inne', 'car', 1),
  (gen_random_uuid(), 'Motocykle', 'moto', 'Motocykle, skutery, quady i pojazdy dwukołowe', 'bike', 2),
  (gen_random_uuid(), 'Części samochodowe', 'parts', 'Części zamienne do samochodów', 'wrench', 3),
  (gen_random_uuid(), 'Akcesoria motoryzacyjne', 'accessories', 'Akcesoria i wyposażenie dla pojazdów', 'tool', 4)
ON CONFLICT (slug) DO NOTHING;

-- Insert packages
INSERT INTO public.packages (id, name, price, duration_days, max_images, description, features, is_popular) VALUES
  (
    gen_random_uuid(),
    'Darmowy', 
    0.00, 
...
-- Insert an initial super admin user
