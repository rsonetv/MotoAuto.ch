...
import { loginAction, signInWithOAuthAction } from "@/app/(auth)/actions"
...
export default function LoginForm() {
  ...
  async function handleOAuth(provider: "google" | "github") {
    try {
      ...
      toast.error(`Wystąpił błąd podczas logowania przez ${provider}`)
    }
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 p-4">
      <Card className="w-full max-w-lg mx-auto shadow-xl border-0">
        <CardHeader className="space-y-1 pb-8">
          ...
          <h1 className="text-2xl font-bold text-center">Witaj ponownie</h1>
          <p className="text-muted-foreground text-center">Zaloguj się do swojego konta</p>
        </CardHeader>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-3">
            ...
            >
              ...
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 0C5.373 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577V20.58c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604C7.58 17.01 4.78 15.98 4.78 11.38c0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23a10.45 10.45 0 0 1 3.003-.404c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.562 21.799 24 17.301 24 12c0-6.627-5.373-12-12-12Z" />
              </svg>
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-background px-2 text-muted-foreground">Lub użyj e-maila</span>
            </div>
          </div>
          ...
          <form action={action} className="space-y-5">
            ...
            <LoadingButton type="submit" className="w-full h-11 font-semibold" loading={isPending} disabled={isPending}>
